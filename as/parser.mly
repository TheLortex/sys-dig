...
| instr=KWD cond=option(conditional) s=option(setflags) rd=REG r1=registre r2=registre rot=option(COMMA rot=ROT i=CST {rot,i}) {
	let cond = encode_cond cond 
	and instr = encode_instr instr 
	and rd = encode_reg rd
	and r1 = encode_reg r1 
	and r2 = encode_reg r2
	and shift = match rot with | Some (rot,i) -> encode_shift rot i| None -> 0
	and s = match s with | Some _ -> 1 | None -> 0 
	in
...
| instr=UNARY cond=option(conditional) s=option(setflags) rd=REG r2=registre rot=option(COMMA rot=ROT i=CST {rot,i}) {
	let cond = encode_cond cond 
	and instr = encode_instr instr 
	and rd = encode_reg rd
	and r1 = 0
