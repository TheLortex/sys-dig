...
let stop_assembly = ref false
let start_assembly = ref false
...
let options = 
  ["-parse-only", Arg.Set parse_only, 
   ...
   "-stop-at-assembly", Arg.Set stop_assembly, " Stop at assembly production";
   "-assembly-to-binary", Arg.Set start_assembly, " Start from assembly";
...
let () = 
  ...
  if not (Filename.check_suffix !ifile ".bamboo") then begin
    eprintf "Le fichier d'entrée doit avoir l'extension .bamboo\n@?";
